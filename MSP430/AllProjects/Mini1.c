...
//     void.
//
void convert_tempr(unsigned int value)
{
	unsigned int temp;
	temp=value/10;
	tempr[0]=temp+48;
...
void main(void)
{
   // WDTCTL = WDTPW + WDTHOLD;             // Stop watchdog timer
	 ...
	 WDTCTL=WDT_ADLY_1000;  // Generate 1 sec delay
	 IE1|=WDTIE;
   ...
   while (1)
   {
      ...
      convert_tempr(CurDegC);



 	 SendByte(0x80,FALSE);  // Display it in UPPER LINE
 	 PrintStr(time);

      SendByte(0xC0,FALSE); // Display it in the LOWER LINE
      PrintStr(tempr);
      __no_operation();
   }
